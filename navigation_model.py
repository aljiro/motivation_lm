...
class NavigationModel(robot_reply_RL.NetworkSetup):
	...
	def __init__(self, tau_elig, eta):
		super().__init__(tau_elig, eta, 1, 1)
		self.t_replay = 0 # s
		self.t_last_command = 0 # s, time since last motor command
		self.coords_prev = self.body_pose[0:2]
		self.theta_prev = self.body_pose[2]
		...
		# ac_average = 0.0
		# total_counter = 0.0
		self.rate = rospy.Rate(int(1 / self.delta_t))
...
# The main portion of the program that starts the ROS loop, runs the MiRo controller and updates all the model variables
	def step(self, dopamine, ach):
		...
		movement_x = coords[0] - self.coords_prev[0]
		movement_y = coords[1] - self.coords_prev[1]
		...
		if  True: #ach == 0: # self.reward_val
			...
			self.action_cell_vals_noise = self.theta_to_action_cell(self.theta_prev)
			...
			# All the variables are called replay, but no replays actually run here... it's just an artifact from
			# the replay model
			if not self.replay:
				print('NOT running reverse replay event')
				self.elig_trace_at_reward = np.zeros((self.network_size_ac, self.network_size_pc))
				for i in range(72):
					for j in range(100):
						if elig_trace_prev[i, j] > 0:
							self.elig_trace_at_reward[i, j] = 0.1
			...
			self.replay = True
			self.t_replay += self.delta_t

			if self.t_replay < 2:
				# No replays in this one I'm afraid
				I_place = np.zeros(self.network_size_pc)

				# Update variables
				# Place cells (should NOT initiate a reverse replay)
				self.currents = self.update_currents(currents_prev, self.delta_t, intrinsic_e_prev,
														network_weights_prev, place_cell_rates_prev, stp_d_prev, stp_f_prev,
				...
				self.weights_pc_ac = self.weight_updates(weights_pc_ac_prev, ach,
															elig_trace_prev,
				...
				self.action_cell_vals_noise = self.action_cell_vals + self.compute_action_cell_outputs(
					weights_pc_ac_prev + self.elig_trace_at_reward, place_cell_rates_prev)

			# if self.t_replay > 2:
				# finish running the NOT replay event, reset variables that need resetting, and go to a random
				# position
				# self.replay = False
				# self.intrinsic_e = self.intrinsic_e_reset.copy()
				# self.elig_trace = np.zeros((self.network_size_ac, self.network_size_pc))
				...
				# self.theta_prev = self.body_pose[2] # resets
		...
		if self.heading_home:
			...
			self.theta_prev = self.body_pose[2] # need to reset for the controller below
		...
		if not self.stop:
			...
			if self.t - self.t_last_command > 0.5: # send a command once every half a second
				...
				if ac_magnitude >= 1:
					...
					self.target_theta = self.random_walk(self.theta_prev, self.target_theta)
			...
			self.miro_controller(self.target_theta, self.theta_prev)
		...
		# 	np.save('data/weight_at_end.npy', self.weights_pc_ac) # save the weights at the end of the experiment
